...
function buildHeaderDropdown(session) {
  ...
  if (session.isAdmin || session.isManagement || session.isDepartmentHead) {
    navItems.push(`<a href="/issues" class="profile-menu-item">
      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
      Issues
    </a>`);
  }
...
let _profileGithubUsername = '';

function renderGithubWidget(githubUsername) {
  _profileGithubUsername = githubUsername || '';
  const widget = document.getElementById('githubLinkWidget');
  if (!widget) return;

  if (_profileGithubUsername) {
    widget.innerHTML = `
      <div class="github-link-connected">
        <img class="github-link-avatar" src="https://github.com/${encodeURIComponent(_profileGithubUsername)}.png?size=64" alt="@${escHtml(_profileGithubUsername)}">
        <div class="github-link-info">
          <a href="https://github.com/${encodeURIComponent(_profileGithubUsername)}" target="_blank" rel="noopener" class="github-link-handle">@${escHtml(_profileGithubUsername)}</a>
          <span class="github-link-status">Connected</span>
        </div>
        <button type="button" class="github-link-unlink" onclick="unlinkGithub()">Disconnect</button>
      </div>`;
  } else {
    widget.innerHTML = `
      <div class="github-link-prompt">
        <span class="github-link-prompt-emoji">🎉</span>
        <div class="github-link-prompt-text">
          <strong>You're a developer!</strong> Link your GitHub account so your avatar, bio, and
          repo/contribution stats show up on your Developer Performance profile.
        </div>
      </div>
      <button type="button" class="github-connect-btn" onclick="connectGithub()">
...
function connectGithub() {
  ...
  window.location.href = `/api/profile/github/connect?u=${encodeURIComponent(session.username)}`;
}

async function unlinkGithub() {
  try {
    const res = await fetch('/api/profile/github', { method: 'DELETE', headers: authHeaders() });
    const data = await res.json();
...
function _handleGithubCallbackParams() {
  const params = new URLSearchParams(location.search);
  if (!params.has('github')) return;
  if (params.get('github') === 'linked') {
    showToast(`GitHub account @${params.get('login') || ''} linked.`);
  } else if (params.get('github') === 'error') {
    showToast(params.get('msg') || 'Failed to link GitHub account.');
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.is_developer) {
        ...
        document.getElementById('githubLinkGroup').style.display = '';
        renderGithubWidget(data.github_username || '');
      }
